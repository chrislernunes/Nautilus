...
def compute_combined_regime(
    ...
    Falls back gracefully to price-only regime.
    """
...
def compute_cross_sectional_momentum(
    ...
    long_w  = pd.DataFrame(0.0, index=prices.index, columns=prices.columns)
    short_w = pd.DataFrame(0.0, index=prices.index, columns=prices.columns)
    ...
    for dt in prices.index:
        ...
        bottom = row.nsmallest(n_short).index
        long_w.loc[dt, top]     = 1.0 / len(top)
        short_w.loc[dt, bottom] = 1.0 / len(bottom)
