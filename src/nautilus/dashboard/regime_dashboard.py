...
""", unsafe_allow_html=True)
    ...
    live_mode     = st.toggle("Live Mode (60 s)", value=False)
    st.markdown("---")
...
if live_mode:
    st.info("**Live Mode** — refreshes every 60 seconds.")
...
@st.cache_data(ttl=3_600, show_spinner=False)
def _load_macro_raw_cached(force, start):
    """Unshifted macro features — passed to fit_hmm so build_hmm_features
    applies a single uniform shift(1) across all features."""
    try:
        nifty = load_index(force_refresh=force)
        df = build_macro_features(nifty["Close"], start=start, force_refresh=force, raw=True)
        return df.to_dict()
    except Exception as exc:
        logger.error("Raw macro load failed: %s", exc); return None
...
macro_raw_dict = _load_macro_raw_cached(refresh, DEFAULT_START_DATE)  # unshifted — for HMM
...
def _stable_hash(*args):
    """Deterministic MD5 hash for arbitrary args including Timestamp-keyed dicts.
    Converts all dict keys to str before JSON serialisation so json.dumps
    never tries to sort/compare mixed-type keys (Timestamp vs str).
    """
    def _sanitise(obj):
    ...
    return hashlib.md5(
        json.dumps(_sanitise(args), sort_keys=True, default=str).encode()
    ).hexdigest()


@st.cache_data(show_spinner=False)
def _fit_hmm_cached(_data_hash: str, pvals, pidx, macro_d, n_iter, use_macro):
...
with st.spinner("Fitting 5-state Gaussian HMM\u2026"):
    _hmm_data_hash = _stable_hash(
        price.tolist(), price.index.astype(str).tolist(),
        macro_raw_dict, use_macro_feat,
        hmm_n_iter,          # ← BUGFIX: changing iterations must bust the cache
    )
    hmm_raw = _fit_hmm_cached(
        _hmm_data_hash,
        price.tolist(), price.index.astype(str).tolist(),
        macro_raw_dict, hmm_n_iter, use_macro_feat,  # raw=unshifted; build_hmm_features shifts uniformly
    )
...
# ── Hard gate (computed once here; reused by Kelly panel and backtest) ──────────
# States 0 (Bull Quiet) = 1.00, 1 (Bull Volatile) = 0.85, 2 (Neutral) = 0.75,
# 3 (Stress) = 0.00, 4 (Panic) = 0.00.  Shifted +1D: gate[t] was decided at
# close(t-1), applied to position held on day t.
_HARD_GATE_MAP = {0: 1.00, 1: 0.85, 2: 0.75, 3: 0.00, 4: 0.00}

if hmm_ok:
    _hard_gate_shifted = (
        hmm_state_s.reindex(price.index).ffill()
        .map(_HARD_GATE_MAP).fillna(1.0)
        .shift(1).fillna(1.0)
    )
else:
    _hard_gate_shifted = pd.Series(1.0, index=price.index)
...
hmm_label  = (
    f"{REGIMES[cur_state]['emoji']} {REGIMES[cur_state]['name']}"
    if hmm_ok else ("\u26a0\ufe0f HMM Error" if _hmm_error_msg else "HMM Disabled")
)
hmm_delta  = (
    f"{cur_probs.max():.0%} conf."
    if hmm_ok else None
)
...
c2.metric("HMM Regime",        hmm_label, delta=hmm_delta)
...
if hmm_ok:
    # ── Hard gate already computed above — use directly ───────────────────
...
if hmm_ok:
    # _hard_gate_shifted computed once above — reuse directly
...
_MK_RETURN  = ["Total Return", "CAGR", "Ann. Vol"]
_MK_RISK    = ["Sharpe", "Sortino", "Calmar", "Profit Factor"]
_MK_DD      = ["Max DD", "Max DD Days", "Longest DD Days", "Avg DD", "DD Episodes", "Recovery Days", "Current DD"]
_MK_TRADE   = ["Win Rate", "Avg Win", "Avg Loss"]
_LOWER_BETTER = {"Max DD", "Max DD Days", "Longest DD Days", "Avg DD",
                 "DD Episodes", "Recovery Days", "Current DD", "Avg Loss", "Ann. Vol"}

def _metric_row(label, v_naive, v_full, v_bh, lower_is_better=False):
    def _raw(s):
        try:
            return float(str(s).replace("%","").replace(",","").replace("d","").strip())
        except Exception:
            return None
    rn, rf = _raw(v_naive), _raw(v_full)
    if rn is None or rf is None or str(v_naive) in ("—","still in DD") or str(v_full) in ("—","still in DD"):
        winner = ""
    elif lower_is_better:
        winner = "✓ Gate" if rf < rn else ("✓ MA" if rn < rf else "=")
    else:
        winner = "✓ Gate" if rf > rn else ("✓ MA" if rn > rf else "=")
    return {"Metric": label, f"MA({ma_window})": v_naive,
            "+ Regime Gate": v_full, "Buy & Hold": v_bh, "Better": winner}
...
for _section, _keys in [
    ...
    perf_rows.append({"Metric": _section, f"MA({ma_window})": "", "+ Regime Gate": "", "Buy & Hold": "", "Better": ""})
    for _k in _keys:
        perf_rows.append(_metric_row(
            _k,
            bt_naive.metrics.get(_k, "—"),
            bt_full.metrics.get(_k,  "—"),
            bt_bh.metrics.get(_k,    "—"),
            lower_is_better=(_k in _LOWER_BETTER),
        ))
perf_df = pd.DataFrame(perf_rows)

col_bt1, col_bt2 = st.columns([2,3])
with col_bt1:
    st.dataframe(perf_df, hide_index=True, use_container_width=True, height=680)
    st.caption(
        ...
        f"*Max DD Days* = calendar days inside the worst single drawdown episode.  \n"
        f"*Longest DD Days* = calendar days in the longest episode by duration.  \n"
        f"*Recovery Days* = days from worst-DD peak to full recovery "
        f"('still in DD' if not yet recovered at end of series)."
    )
with col_bt2:
...
_gate = float(_hard_gate_shifted.iloc[-1])
...
with st.expander("Methodology — Model Design, Data Sources & No Look-Ahead"):
    st.markdown(f"""
...
hard_gate(t) = argmax P(state | data_{{1..t}}) \u2192 mult
  Bull Quiet    = 1.00   Bull Volatile = 0.85
  Neutral       = 0.75
  Stress        = 0.00   Panic         = 0.00
```
