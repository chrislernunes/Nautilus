...
v5 redesign: regime multipliers are RISK GATES, not Kelly fractions.
    Bull Quiet    = 1.00 (full exposure — low volatility bull)
    Bull Volatile = 0.85 (mild haircut — elevated volatility warrants
                          slightly lower leverage even in uptrends)
    Neutral       = 0.75 (mild reduction)
    Stress        = 0.35 (meaningful cut)
    Panic         = 0.00 (cash)

v5.1 fix — Bull Quiet / Bull Volatile differentiation
──────────────────────────────────────────────────────
Both states were previously assigned mult=1.00, making them
strategically identical. The HMM wasted capacity learning two states
with the same sizing implication. Bull Volatile now uses 0.85 to
reflect that elevated volatility in a bull environment warrants a
modest exposure reduction, while preserving the uptrend participation.
"""
...
REGIMES: dict[int, dict] = {
    ...
    1: {"name": "Bull Volatile", "color": "#3498DB", "mult": 0.85, "cap": 1.20, "emoji": "🔵"},
...
def build_hmm_features(
    ...
    IMPORTANT — `macro_df` must be RAW / unshifted.
    A single shift(1) is applied to the entire matrix at the end so that
    price-derived and macro features share an identical 1-day lag.
    Do NOT pass the shifted output of build_macro_features() here — that
    would double-shift macro columns, misaligning them by 1 day vs price
    features and corrupting the HMM covariance structure.
    """
    ...
    if macro_df is not None:
        ...
        for col in _macro_cols:
            if col in macro_df.columns:
                # macro_df must be RAW (unshifted) — shift(1) is applied
                # below uniformly for ALL features.  build_macro_features()
                # previously returned a pre-shifted frame causing macro cols
                # to see 2 shifts vs 1 for price features — corrupting the
                # HMM covariance structure.  Caller must pass raw macro.
                df[col] = macro_df[col].reindex(price.index)

    # Single shift(1) here — every feature (price-derived and macro) sees
    # exactly one 1-day lag, keeping the feature matrix temporally consistent.
